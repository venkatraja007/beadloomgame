...
public class GUIInputTools extends JApplet implements ActionListener, ItemListener{
	...
	int GRID_SIZE = 30; // default grid size
    int PAD       = 20;
	double xInc;
	double yInc;
...
//			======== LayersTabbedPane ========
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == DrawPointButton) 
		{
			...
			} catch(Exception exc){JOptionPane.showMessageDialog(null, "Values must be integers between -50 and 50");}
		...
		else if(e.getSource() == DrawLineButton) 
		{
			...
			try {
				...
				//If the arguments are not in bounds throw an exception
				if(x1 < -50 || x1 > 50 || y1 > 50 || y1 < -50 || x2 > 50 || x2 < -50 ||
						y2 > 50 || y2 < -50)
					throw new NumberFormatException();
				
				//Initialize the coordinate list and layer
				CoordList coords = new CoordList(xValue, yValue);
				Layer l = new Layer("LINE", coords, " ");
				l.setColor(color);
				l.setImage(makeBullet(l.getColor()));
				grid.addLayer(l);
		...
		//Utilizes the drawTriangle method
			...
			try {
				...
				//If the arguments are not in bounds throw an exception
				if(x1 < -50 || x1 > 50 || y1 > 50 || y1 < -50 || x2 > 50 || x2 < -50 ||
						y2 > 50 || y2 < -50 || x3 > 50 || x3 < -50 || y3 > 50 || y3 < -50)
					throw new NumberFormatException();
		...
		else if(e.getSource() == DrawTriangleIterationButton) 
		{
			...
			try {
				...
				if(width < 0 || height < 0 || cycles < 0)
					throw new NumberFormatException();
				if(startX < -50 || startX > 50 || startY < -50 || startY > 50 ||
						width > 50 || height > 50 || height < -50 || cycles > 50)
					throw new NumberFormatException();
			}
			catch(Exception exc)
			{
			...
			//Draw the iteration
			CoordList coords = new CoordList(xValue, yValue);
			Layer l = new Layer("LINEAR_ITERATION", coords," ");
			l.setColor(color);
			l.setImage(makeBullet(l.getColor()));
			grid.addLayer(l);
		}
		...
		else if(e.getSource() == DrawLinearIterationButton)
		{
			...
			try
			{
				...
				//If the arguments are not in bounds throw an exception
				if(startX < -50 || startX > 50 || startY < -50 || startY > 50 || startLength > 50 ||
						rows > 50 || inc1 < -50 || inc1 > 50 || inc2 < -50 || inc2 > 50)
					throw new NumberFormatException();
			}
			catch(Exception exc){JOptionPane.showMessageDialog(null, "Values must be integers between -50 and 50.");}
			...
			//Draw the iteration
			CoordList coords = new CoordList(xValue, yValue);
			Layer l = new Layer("TRIANGLE_ITERATION", coords," ");
			l.setColor(color);
			l.setImage(makeBullet(l.getColor()));
			grid.addLayer(l);
		}
    ...
    public JTextField getTriangleIterationStepsTextField(){
    	return TriangleIterationStepHeightTextField;
    }
    public JTextField getTriangleIterationWidthTextField(){
    	return TriangleIterationBeadsAddedTextField;
    }
    public JTextField getTriangleIterationCyclesTextField(){
